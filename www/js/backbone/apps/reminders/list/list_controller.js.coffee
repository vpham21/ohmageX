      ...
      @listenTo @layout, "show", =>
        ...
        if permissions.get('localNotification') is true
          ...
          if surveys.length is 0
            @noticeRegion "No saved #{App.dictionary('pages','survey')}! Download #{App.dictionary('pages','campaign')} from the #{App.dictionary('page','campaign').capitalizeFirstLetter()} Menu section to create new #{App.dictionary('pages','reminder')}."
          else
            @initBlockerView()
